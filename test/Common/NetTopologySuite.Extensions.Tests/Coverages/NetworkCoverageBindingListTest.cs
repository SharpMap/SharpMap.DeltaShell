...
namespace NetTopologySuite.Extensions.Tests.Coverages
{
    ...
    public class NetworkCoverageBindingListTest
    {
        ...
        public void CanGetCorrectValuesFromRowUsingIndexer()
        {
            ...
            Assert.AreEqual(networkCoverage.Locations.Values[0].Chainage, firstRow[1]);
        }
        ...
        [Test]
        public void CanSetChainageOnLocationUsingIndexer()
        {
            var firstRow = networkCoverageBindingList[0];

            var newChainage = 20.0;

            Assert.AreNotEqual(newChainage, networkCoverage.Locations.Values[1].Chainage);

            firstRow[1] = newChainage;

            firstRow.EndEdit(); //commit

            Assert.AreEqual(newChainage, networkCoverage.Locations.Values[1].Chainage); //sorting also occurs
        }
        ...
        public void CanGetCorrectValuesFromRowUsingColumnNameIndexer()
        {
            ...
            Assert.AreEqual(networkCoverage.Locations.Values[0].Chainage, firstRow[NetworkCoverageBindingList.ColumnNameChainage]);
        }
        ...
        public void CanGetCorrectValuesFromItemProperties()
        {
            ...
            var firstOffset = ((NetworkLocation) networkCoverage.Arguments[0].Values[0]).Chainage;
        ...
        public void CanSetValuesThroughItemProperties()
        {
            ...
            firstRow.EndEdit(); //commit
            firstRow = networkCoverageBindingList[0];
            ...
            firstRow.EndEdit(); //commit
            firstRow = networkCoverageBindingList[0];
            Assert.AreEqual(newOffset, ((NetworkLocation)networkCoverage.Arguments[0].Values[0]).Chainage);
