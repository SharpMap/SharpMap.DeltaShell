    ...
    public class VariableValueFilterTest
    {
        ...
        public void ConstructorPreconditions()
        {
            ...
            catch (Exception e)
            {
                ...
                Assert.That(e.Message.StartsWith("Cannot be empty\r\n"), Is.True);
                Assert.That(((ArgumentOutOfRangeException)e).ParamName, Is.EqualTo("values"));
            ...
            {
                Assert.That(e, Is.TypeOf<ArgumentOutOfRangeException>());
                Assert.That(e.Message.StartsWith("Invalid value type\r\n"), Is.True);
                Assert.That(((ArgumentOutOfRangeException)e).ParamName, Is.EqualTo("values"));
                //Assert.AreEqual(typeof(ArgumentOutOfRangeException), e.GetType());
