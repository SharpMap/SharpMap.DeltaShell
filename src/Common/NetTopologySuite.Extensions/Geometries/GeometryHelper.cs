...
namespace NetTopologySuite.Extensions.Geometries
{
    public static class GeometryHelper
    {
        ...
        ///  - IGeometry.Coordinates converts an internal ICoordinateSequence to an Coordinate array
        ...
        /// <returns></returns>
        public static double Distance(ILineString lineString, Coordinate worldPos)
        {
            // Distance along linestring
            double minDistance = Double.MaxValue;
            if (lineString == null || worldPos == null) return minDistance;

            Coordinate min_c1 = null;
            Coordinate min_c2 = null;
            ...
            if (-1 != index)
            {
                Coordinate location = NearestPointAtSegment(min_c1.X, min_c1.Y,
...
*/
        ...
        public static IGeometry SetCoordinate(IGeometry geometry, int coordinateIndex, Coordinate coordinate)
        {
        ...
        public static void MoveCoordinate(Coordinate[] targetCoordinates, Coordinate[] sourceCoordinates, int coordinateIndex, double deltaX, double deltaY)
        {
        ...
        public static ILineString CreateLineString(IPoint start, IPoint end, int numberOfSegments)
        {
            var coordinates = new List<Coordinate>(numberOfSegments + 1);
            coordinates.Add(start.Coordinate);
            for (int i = 1; i < numberOfSegments; ++i)
            {
                var x = start.X + i * (end.X - start.X) / numberOfSegments;
                var y = start.Y + i * (end.Y - start.Y) / numberOfSegments;
                coordinates.Add(new Coordinate(x, y));
            }
            coordinates.Add(end.Coordinate);
        ...
        public static Coordinate GetNearestPointAtLine(ILineString lineString, Coordinate coordinate, double tolerance, out int snapVertexIndex)
        {
            snapVertexIndex = -1; 
        ...
        public static Coordinate GetNearestPointAtLine(ILineString geometry, Coordinate coordinate, double tolerance)
        {
            int snapVertexIndex;
            return GetNearestPointAtLine(geometry, coordinate, tolerance, out snapVertexIndex);
        }

        public static IFeature GetNearestFeature(Coordinate coordinate, IEnumerable<IFeature> features, double tolerance)
        {
        ...
        public static IEnumerable<IFeature> GetFeaturesInRange(Coordinate coordinate, IEnumerable<IFeature> features, double tolerance)
        {
            var minDistance = tolerance;
            var point = new Point(coordinate);
        ...
        private static bool RemoveRedundantPoint(List<Coordinate> resultCoordinates)
        {
            var removed = false;
        ...
        public static bool PointIsSame(Coordinate point,Coordinate previousPoint)
        {
            return point.X == previousPoint.X && point.Y == previousPoint.Y;
        }

        public static bool PointIsOnLineBetweenPreviousAndNext(Coordinate previousPoint, Coordinate point, Coordinate nextPoint)
        {
            //vertical
        ...
        public static IGeometry InsertCurvePoint(IGeometry geometry, Coordinate coordinate, int index)
        {
            var vertices = new List<Coordinate>(geometry.Coordinates);
