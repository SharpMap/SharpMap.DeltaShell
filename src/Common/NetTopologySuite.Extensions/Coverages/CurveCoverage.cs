    ...
    public class CurveCoverage : Coverage, ICurveCoverage
    {
        ...
        public virtual double GetInterpolatedValue(params IVariableFilter[] filters)
        {
            ...
            IList<IFeatureLocation> branchLocations =
                networkLocations.Where(bf => bf.Feature == networkLocation.Feature).OrderBy(bf => bf.Chainage).ToList();
            ...
            {
                x[i + 1] = branchLocations[i].Chainage;
            }
            ...
            /*double []y,*/ double chainage)
        {
            ...
            for (int i = 1; i < x.Length; i++)
            {
                ...
                return (yiminus1 + (chainage - x[i - 1]) * (yi - yiminus1) / (x[i] - x[i - 1]));
            }
            // or return y[y.Length - 1]?
            throw new ArgumentException(string.Format("NetworkCoverage ({0}): Chainage ({1}) out of range; can not interpolate", Name, chainage), "chainage");
        }
        ...
        /// <param name="x">
        /// Array with chainages of the branch location. An extra chainage 0 is added in front, at the end an extra chainage for
        /// the length of the branch.
        /// </param>
        /// <param name="chainage">
        /// The chainage for which the value has to interpolated
        /// </param>
        /// <returns>
        /// The interpolated value
        /// </returns>
        private double Interpolate(IList<IFeatureLocation> branchLocations, double[] x, double chainage)
        {
            if (chainage > (x.Max() + errorMargin))
            {
                throw new InvalidOperationException(
                    string.Format(
                        "NetworkCoverage ({0}): Chainage ({1}) exceeds branch length ({2}). Interpolation impossible",
                        Name, chainage, x.Max()));
            }
