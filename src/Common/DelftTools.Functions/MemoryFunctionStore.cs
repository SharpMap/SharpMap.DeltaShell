    ...
    public class MemoryFunctionStore : Unique<long>, IFunctionStore, INotifyPropertyChange
    {
        ...
        private IList<IMultiDimensionalArray> FunctionValues
        {
            get
            {
                ...
                {
                    SubscribeToFunctionValues(this.functionValues);
            ...
            set
            {
                UnSubscribeToFunctionValues(this.functionValues);
                this.functionValues = value;
        ...
        public virtual void RemoveFunctionValues(IFunction function, params IVariableValueFilter[] filters)
        {
            ...
            if(filters.Length == 0) // optimized version
            {
                ...
                {
/*
                    if (!(function is IVariable)) // clear all - performance optimization
                    {
                        var arrays = functionValues.Where(a => a != null).ToArray();

                        arrays.ForEach(a => a.FireEvents = false);
                        arrays.ForEach(a => a.Clear());
                        arrays.ForEach(a => a.FireEvents = true);

                        if (FunctionValuesChanging != null)
                        {
                            foreach (var array in arrays)
                            {
                                FunctionValuesChanging(array, new FunctionValuesChangingEventArgs { Action = NotifyCollectionChangeAction.Reset });
                            }
                        }
                        if (FunctionValuesChanged != null)
                        {
                            foreach (var array in arrays)
                            {
                                FunctionValuesChanged(array, new FunctionValuesChangingEventArgs { Action = NotifyCollectionChangeAction.Reset });
                            }
                        }
...
*/
                ...
                /* performance optimized version of the above if - FINISH IT
                if (function.IsIndependent && functions.Count(f => f.IsIndependent) > 1)
                {
                    foreach (var variable in function.Components)
                    {
                        GetValues(variable).Clear();
                ...
                else // function with a 1 argument, clearing argument values or component values
                {
                    var arrays = functionValues.Where(a => a != null).ToArray();
                    foreach (var array in arrays)
                    {
        ...
        public virtual bool SupportsPartialRemove { get { return true; } }
        ...
        public virtual T GetMaxValue<T>(IVariable variable)
        {
            ...
            return maxValue is IConvertible ? (T) Convert.ChangeType(maxValue, typeof (T)) : default(T);
        }
        ...
        public virtual T GetMinValue<T>(IVariable variable)
        {
            ...
            return minValue is IConvertible ? (T) Convert.ChangeType(minValue, typeof (T)) : default(T);
        }
        ...
        public virtual bool SkipChildItemEventBubbling
        {
            get;
            set;
        }
        ...
        // HACK: store and Function, Variable relations are broken
        bool INotifyPropertyChange.HasParent { get { return false; } set { } }
        ...
        private readonly IDictionary<int, IVariable> arrayHashToVariableLookup = new Dictionary<int, IVariable>();
        private bool doNotCheckForDependentVariableResize;
        ...
        private void RebuildArrayToVariableLookup()
        {
            arrayHashToVariableLookup.Clear();

            for (int i = 0; i < FunctionValues.Count; i++ )
            {
                var array = FunctionValues[i];

                if (array == null) //happens for functions
                    continue;

                var variable = (IVariable)functions[i];
                arrayHashToVariableLookup.Add(RuntimeHelpers.GetHashCode(array), variable);
            }
        }
        ...
        {
            if (SkipChildItemEventBubbling && (!(sender is IMultiDimensionalArray)))
                return;
