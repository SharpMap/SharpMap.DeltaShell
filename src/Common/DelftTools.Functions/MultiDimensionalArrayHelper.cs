...
namespace DelftTools.Functions
{
    public class MultiDimensionalArrayHelper
    {
        ...
        /// </summary>
        public static int GetInsertionIndex(object obj, IList sortedList)
        {
            var theObject = (IComparable) obj;

            if (sortedList.Count == 0 || !theObject.IsBigger((IComparable)sortedList[0])) //if the object is smaller than the 1st we add it to the start.
            {
                return 0;
            }

            if (theObject.IsBigger((IComparable) sortedList[sortedList.Count - 1])) //if the object is larger than the last, we extend the sortedList.
            {
