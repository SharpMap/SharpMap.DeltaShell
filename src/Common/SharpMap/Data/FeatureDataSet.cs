...
namespace SharpMap.Data
{
	...
	{
	    public static FeatureDataTable FromFeatures(IList features)
	    {
            if (features == null || features.Count == 0)
                throw new ArgumentException(nameof(features));

            var fdt = CreateFeatureDataTable((IFeature)features[0]);
            fdt.BeginLoadData();
            object[] tmp = new object[fdt.Columns.Count];
	        for (var i = 0; i < features.Count; i++)
	        {
	            var values = ((IFeature) features[i]).Attributes.Values;
                values.CopyTo(tmp, 0);
	            var fdr = (FeatureDataRow) fdt.LoadDataRow(tmp, LoadOption.OverwriteChanges);
	            fdr.Geometry = ((IFeature) features[i]).Geometry;
	        }
            fdt.EndLoadData();
	        return fdt;
	    }

	    private static FeatureDataTable CreateFeatureDataTable(IFeature feature)
	    {
	        var res = new FeatureDataTable();
            res.BeginInit();
	        foreach (string key in feature.Attributes.Keys)
	        {
	            object value = feature.Attributes[key];
	            res.Columns.Add(key, value?.GetType() ?? typeof(object));
	        }
            res.EndInit();
	        return res;
	    }
