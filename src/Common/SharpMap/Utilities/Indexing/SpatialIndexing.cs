...
namespace SharpMap.Utilities.SpatialIndexing
{
	...
	public class QuadTreeOld : IDisposable
	{
		...
		private Envelope _box = new Envelope();
...
//				// bucket bbox based on their midpoint's side of the geo average in the longest axis
		...
		/// </summary>
		/// <returns></returns>
		public double ErrorMetric(Envelope box)
		{
			Coordinate temp = GeometryFactoryEx.CreateCoordinate(1 + (box.MaxX-box.MinX), 1 + (box.MaxY-box.MinY));
		...
		public Collection<int> Search(Envelope box)
		{
		...
		private void IntersectTreeRecursive(Envelope box, QuadTreeOld node, ref Collection<int> list)
		{
