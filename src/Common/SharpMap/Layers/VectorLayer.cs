...
namespace SharpMap.Layers
{
    ...
    public class VectorLayer : Layer, IDisposable
    {
        ...
        protected override void UpdateCurrentTheme()
        {
            ...
            {
                double min, max;
                GetDataMinMax(gradientTheme.AttributeName, out min, out max);
                gradientTheme.Min = min;
                gradientTheme.Max = max;
                gradientTheme.UpdateThemeItems();
            }
        }

        private void GetDataMinMax(string attributeName, out double min, out double max)
        {
            if (!string.IsNullOrEmpty(ThemeGroup))
            {
                ((Map)Map).GetDataMinMaxForThemeGroup(ThemeGroup, attributeName, out min, out max);
                return;
            }
            min = MinDataValue;
            max = MaxDataValue;
        ...
        */
        ...
        private void RenderFeatures(Graphics g, Envelope envelope, IMap map)
        {
