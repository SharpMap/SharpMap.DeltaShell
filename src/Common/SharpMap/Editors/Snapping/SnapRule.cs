...
namespace SharpMap.Editors.Snapping
{
    public class SnapRule : ISnapRule
    {
        ...
        /// <returns></returns>
        public virtual SnapResult Execute(IFeature sourceFeature, IFeature[] snapCandidates, ILayer[] snapLayers, IGeometry sourceGeometry, IList<IFeature> snapTargets, Coordinate worldPos, Envelope envelope, int trackingIndex)
        {
        ...
        public void LineStringSnapAllTrackers(ref double minDistance, ref SnapResult snapResult, ILineString lineString, Coordinate worldPos)
        {
        ...
        public void LineStringSnapFree(ref double minDistance, ref SnapResult snapResult, ILineString lineString, Coordinate worldPos)
        {
            for (int i = 1; i < lineString.Coordinates.Length; i++)
            {
                Coordinate c1 = lineString.Coordinates[i - 1];
                Coordinate c2 = lineString.Coordinates[i];
                double distance = GeometryHelper.LinePointDistance(c1.X, c1.Y, c2.X, c2.Y,
                                                                   worldPos.X, worldPos.Y);
                if (distance >= minDistance)
                    continue;
                minDistance = distance;
                snapResult = new SnapResult(GeometryFactoryEx.CreateCoordinate(worldPos.X, worldPos.Y), null, null, lineString, i - 1, i) { Rule = this };
            }
        }
