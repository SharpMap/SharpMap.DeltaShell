...
namespace SharpMap.UI.Tools
{
    ...
    public class SnapTool : MapTool
    {
        ...
        public void AddSnap(IGeometry geometry, Coordinate from, Coordinate to)
        {
            var vertices = new List<Coordinate> {from, to};
        ...
        /// <returns></returns>
        public SnapResult ExecuteSnapRule(ISnapRule snapRule, IFeature sourceFeature, IGeometry snapSource, IList<IFeature> snapTargets, Coordinate worldPos, int trackingIndex)
        {
            float marge = (float)MapHelper.ImageToWorld(Map, snapRule.PixelGravity);
            Envelope envelope = MapHelper.GetEnvelope(worldPos, marge);
        ...
        IEnumerable<DelftTools.Utils.Tuple<IFeature, ILayer>> GetSnapCandidates(Envelope envelope)
        {
        ...
        public SnapResult ExecuteLayerSnapRules(ILayer sourceLayer, IFeature feature, IGeometry geometry, 
                                                Coordinate worldPosition, int trackerIndex)
        {
