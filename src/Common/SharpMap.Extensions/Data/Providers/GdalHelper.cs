    ...
    public class GdalHelper
    {
                                               ...
                                               int imageWidth, int imageHeight, T[] values)
        {
            ...
            using (Band band = dataset.GetRasterBand(bandIndex))
            {
                ...
                var pinnedHandle = GCHandle.Alloc(values, GCHandleType.Pinned);
                var size = Marshal.SizeOf(typeof(T));
                var dataType = GetGdalDataType(dataset.GetDriver(), typeof(T));
                CPLErr err;

                try
                {
                    err = band.WriteRaster(xOff, yOff, width, height, pinnedHandle.AddrOfPinnedObject(), imageWidth, imageHeight, dataType, size, 0);
                }
                finally
                {
                    pinnedHandle.Free();
                                              ...
                                              int imageWidth, int imageHeight)
        {
            ...
            using (Band band = dataset.GetRasterBand(bandIndex))
            {
                ...
                try
                {
                    band.ReadRaster(xOff, yOff, width, height, pinnedHandle.AddrOfPinnedObject(), imageWidth, imageHeight, dataType, size, 0);
                }
                finally
                {
                    pinnedHandle.Free();
